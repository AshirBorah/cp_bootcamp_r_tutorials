...
## Some notes on using color {.small-bullets}
...
```{r convert_to_factor, exercise=F, fig.width = 4.5, fig.height = 3}
my_df <- gapminder %>%
...
```{r facets, exercise=F, exercise.setup='facet_setup', fig.width = 4.5, fig.height = 3}
...
```{r add_text_plot, fig.width = 4.5, fig.height = 3}
...
Or with `geom_label`
```{r add_label_plot, fig.width = 4.5, fig.height = 3.5}
ggplot(gap_df, mapping = aes(x = gdp, y = lifeExp, label = country)) +
  geom_label() +
  geom_point() +
  geom_smooth(method = 'lm', se = FALSE) +
...
```{r ggrepel, fig.width = 5, fig.height = 3}
library(ggrepel)
...
```{r plot_beauty, fig.width = 5, fig.height = 4}
...
-   See VERY detailed documentation with examples [here](https://jokergoo.github.io/ComplexHeatmap-reference/book/more-examples.html#add-more-information-for-gene-expression-matrix)
