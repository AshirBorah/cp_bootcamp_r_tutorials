...
```{r}
get_usable_genes <- function(counts_mat, *FILL_IN*) {
  counts_per_million <- cpm(counts_mat) #normalize by total counts (to give units of 'counts-per-million')
...
First, let's make a new column in the sample info table called `is_basal` which is true if the sample is group Basal, and FALSE otherwise. You can add a new column to a table like this: `df[['new_column']] <- new_vector`
```{r}
