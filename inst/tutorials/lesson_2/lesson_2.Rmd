...
```{r function_syntax, exercise = TRUE}
...
```{r func_no_output, exercise = TRUE}
##NO RUN
save(object_to_save, file = 'data_file')
```

```{r func_no_input, exercise = TRUE}
getwd()
```
...
```{r help_syntax, exercise = TRUE}
##NO RUN
?my_function
```

```{r help_length, exercise = TRUE}
?length()
```
...
```{r positional_arg_example, exercuse = FALSE}
...
```{r biocmanager_pkg, exercise = FALSE}
...
```{r pkg_namespace-setup}
library(lubridate)
library(here)
```

```{r pkg_namespace, exercise = T}
?lubridate::here()
?here::here()
```
...
```{r function_format_review, exercise = T}
## NO RUN
